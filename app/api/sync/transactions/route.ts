...
async function fetchCreditTransactions(startDate: string, endDate: string): Promise<ApiTx[]> {
  const all: ApiTx[] = [];
  let page = 1;
  const limit = 100;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const lastRes = await supabaseFetch(
      "/rest/v1/credit_manager_transactions?select=created_at&type=eq.credit&order=created_at.desc&limit=1"
    );
    ...
    console.log(`Syncing transactions (credit) from ${start} to ${end}...`);

    const transactions = await fetchCreditTransactions(start, end);
    console.log(`Fetched ${transactions.length} credit transactions from API`);
    ...
    if (transactions.length === 0) {
      return NextResponse.json({
        ...
        success_count: 0,
        error_count: 0,
        message: "No credit transactions to sync",
      });
    }
